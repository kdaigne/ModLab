...
\date{Last update of the user guide: October 25, 2024}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{gui_scheme.pdf}
...
\par
See example\_simu\_to\_select (file must be unzipped) for a simulation example.

\section{Structure}

The GUI is made up of several interconnected blocks, as shown in \cref{modlabArch}.
...
\verb|'.','/',':',';', ...|\\
\verb|'<','=','>','?','@','\','^','_','`','''','{','}','~',']'},'_'))];|\\
\verb|LauncherName=[LauncherName format];|\\
\newline
The result is:\\
\verb|LauncherName = 'Launcher_example_fr.sh'|
